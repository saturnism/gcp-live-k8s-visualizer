        ...
        for (var j = 0; j < pods.items.length; j++) {
            ...
            if (matchesLabelQuery(pod.metadata.labels, service.spec.selector)) {
                ...
                jsPlumb.connect({
                    ...
                    endpointStyle: {fillStyle: 'rgb(0,153replicationcontrollers,57)', radius: 7},
                    connector: ["Flowchart", {cornerRadius: 5}]
                });
            }
        }
        ...
        $.each(list, function (index, value) {
            ...
            if (value.type == "pod") {
                ...
                eltDiv.html('<span>' +
                    ...
                    (value.status.podIP ? "<br/><br/>" + value.status.podIP : "") +
                    (value.spec.nodeName ? " (" + value.spec.nodeName + ")" : "") +
